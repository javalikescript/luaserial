...
static int setSerial(lua_State *l) {
  ...
  if (lua_istable(l, 2)) {
    ...
    init = getBooleanField(l, 2, "initialize", 1);
    canonicalMode = getBooleanField(l, 2, "canonicalMode", 1);
    clear = getBooleanField(l, 2, "clear", 0);
  } else {
    ...
    init = getBoolean(l, 6, 1);
    canonicalMode = getBoolean(l, 7, 1);
  ...
  if (init) {
    ...
    term.c_cc[VTIME] = 0; // inter-character timer unused
